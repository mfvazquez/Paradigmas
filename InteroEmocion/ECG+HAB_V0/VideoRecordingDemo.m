function VideoRecordingDemo(moviename,video_time, codec, withsound, showit, windowed)
...
% screen2=max(Screen('Screens'));
%  w=Screen('OpenWindow', screen2, 0);
%  
% 
%  
...
if nargin < 1
    error('You must provide an output movie name as first argument!');
end
...
try
    if windowed > 0
        ...
        [win, screen_size]=Screen('OpenWindow', screen, 0);
...
% Screen('FillRect', w , red, record_rectangle);
% Screen('Flip', w,[],1, [], 1);
    Screen('Flip',win,[], 1,[], 1);
    ...
    if IsWin
        ...
        grabber = Screen('OpenVideoCapture', win, 20000, [0 0 640 480],[] ,[], [] , codec, withsound);
    else
        % No need for Windows-style workarounds:
        grabber = Screen('OpenVideoCapture', win, 20000, [],[] ,[], [] , codec, withsound);
    end
...
for nreps = 1:1
    ...
    while GetSecs-t<(video_time*60)
        ...
        if waitforimage~=4
            ...
            record_rectangle= [screen_size(3)-125 50 screen_size(3)-50 125];
            red= [204 0 0];
            black= [0 0 0];
...
% %                 Screen('DrawText', win, sprintf('Capture time (secs): %.4f', pts), 20, 0, 255);
%                 if count>0
%                     % Compute delta between consecutive frames:
% %                     delta = (pts - oldpts) * 1000;
% %                     oldpts = pts;
% %                     Screen('DrawText', win, sprintf('Interframe delta (msecs): %.4f', delta), 20, 20, 255);
%                     
%                 end

                Screen('FillRect',win, red, record_rectangle);
